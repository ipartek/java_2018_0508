...
INSERT INTO `alumno` VALUES (6,'Adrian Garcia Santos'),(1,'Adrian Perozzo Diaz'),(10,'Adriana Prado Alonso'),(4,'Ainara Goitia Arenaza'),(2,'Alain Muñoz Arrizabalaga'),(9,'Andrea Maria Perez Millan'),(5,'Asier Cornejo Panduro'),(7,'Carlos Léon Montero'),(12,'Eneko Sanchez Retolaza'),(14,'Guillermo Sánchez Zabala'),(11,'Joseba Ramirez Freire'),(16,'Nuevo Alumno'),(8,'Raquel Pastor Villarroel'),(3,'Raul Abejon Delgado'),(13,'Valeria Valencia García');
...
INSERT INTO `editorial` VALUES (9,'alfaguara2'),(3,'Allende'),(2,'AMAYA YA'),(43,'Cámbiate joder'),(49,'Editorial Mock'),(40,'La Nueva'),(1,'Modificadooo'),(35,'Pepito'),(44,'Yahooo');
...
INSERT INTO `libro` VALUES (2,'Moby Dick','12345678912',2),(5,'Peter Pan','12349875-N',3),(6,'Peter Pan','12349875-N',3),(11,'Eliminar II ','12345678912',2),(13,'El libro de la selva','12345-a',1),(14,'El libro de la selva','12345-a',1),(15,'El libro de la selva','12345-a',1),(16,'El libro de la selva','12345-a',9),(17,'El libro de la selva','12345-a',1),(18,'El libro de la selva','12345-a',1),(19,'El libro de la selva','12345-a',1),(20,'El libro de la selva','12345-a',1),(21,'El libro de la selva','12345-a',1),(22,'El libro de la selva','12345-a',1),(23,'El lkibro de la selava andrea','12345',1),(24,'El lkibro de la selava andrea','12345',3),(26,'Nuevo','1234562345y',3),(27,'Nuevo editado','1234562345y',3),(28,'Moby Dick','12345678912344',1),(29,'Moby Dick II','12345678912344',1),(30,'Moby dick','12345678912345',1),(31,'Moby dick','12345678912345',1),(32,'Moby dick','12345678912345',1),(33,'Moby dick','12345678912345',1),(38,'Moby Dick','12345678912344',1),(40,'PEPE','12345678912344',1),(41,'PEPE','12345678912344',2);
...
INSERT INTO `prestamo` VALUES (15,1,'2018-11-11','2018-11-26','2018-11-25'),(30,4,'2018-11-12','2018-11-26','2018-11-27'),(31,5,'2018-11-30','2018-11-16','2018-11-28');
...
,
	OUT `o_id_libro` INT,
    OUT `o_editorial_nombre` VARCHAR(45)
...
SET o_id_libro = LAST_INSERT_ID();

SET o_editorial_nombre :=  (SELECT nombre FROM editorial WHERE ideditorial = p_id_editorial);
...
BEGIN
UPDATE libro 
SET titulo = p_titulo, isbn = p_isbn, id_editorial = p_editorial WHERE idlibro = p_id;

SET o_editorial_nombre :=  (SELECT nombre FROM editorial WHERE ideditorial = p_editorial);
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
,
	OUT `o_alumno_ok` BOOLEAN, 
    OUT `o_libro_ok` BOOLEAN,
    OUT `o_fecha_fin` DATE, 
    OUT `o_editorial_id` INT,
    OUT `o_editorial_nombre` VARCHAR(45),
    OUT `o_libro_titulo` VARCHAR(150),
    OUT `o_libro_isbn` VARCHAR(20),
    OUT `o_alumno_nombre` VARCHAR(45)
)
BEGIN


IF ((SELECT COUNT(id_libro) FROM prestamo WHERE id_libro = p_libro AND fecha_retorno IS NULL) = 0)
THEN SET o_libro_ok := TRUE;
ELSE  SET o_libro_ok := FALSE;
END IF;

IF ((SELECT COUNT(id_alumno) FROM prestamo WHERE id_alumno = p_alumno AND fecha_retorno IS NULL) = 0) THEN 
SET o_alumno_ok := TRUE;
ELSE SET o_alumno_ok := FALSE;
END IF;

IF (o_libro_ok = TRUE && o_alumno_ok = TRUE)
THEN 
...
SET o_fecha_fin := (SELECT fecha_fin FROM prestamo WHERE id_libro = p_libro AND id_alumno = p_alumno AND fecha_inicio = p_fecha );

SET o_editorial_id := (SELECT id_editorial FROM libro WHERE idlibro = p_libro);
SET o_editorial_nombre := (SELECT nombre FROM editorial WHERE ideditorial = o_editorial_id);

SET o_alumno_nombre := (SELECT nombre FROM alumno WHERE idalumno = p_alumno);

SET o_libro_titulo := (SELECT titulo FROM libro WHERE idlibro = p_libro);
SET o_libro_isbn := (SELECT isbn FROM libro WHERE idlibro = p_libro);
...
BEGIN
SELECT p.id_alumno, p.id_libro, p.fecha_inicio, p.fecha_fin,fecha_retorno
FROM prestamo as p;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
SELECT 
a.nombre, p.id_alumno, l.titulo, p.id_libro, p.fecha_inicio, p.fecha_fin, p.fecha_retorno, l.isbn, l.id_editorial, e.nombre as 'editorial'
FROM prestamo as p
INNER JOIN alumno as a ON a.idalumno = p.id_alumno
INNER JOIN libro as l ON l.idlibro = p.id_libro
INNER JOIN editorial as e ON l.id_editorial = ideditorial
...
IF (o_libro_ok = TRUE && o_alumno_ok = TRUE)
THEN INSERT INTO prestamo (id_libro, id_alumno, fecha_inicio) VALUES (p_libro, p_alumno, p_fecha);
...
SET o_fecha_fin := (SELECT fecha_fin FROM prestamo WHERE id_libro = p_libro AND id_alumno = p_alumno AND fecha_inicio = p_inicio );
...
-- Dump completed on 2018-11-19  9:22:02
