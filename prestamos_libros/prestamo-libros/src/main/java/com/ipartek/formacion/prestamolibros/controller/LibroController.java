			...
			if ("".equals(libro.getTitulo().trim()) || "".equals(libro.getIsbn().trim())) {
				...
				if (libro.getId() == -1) {
						...
						if (servicioLibro.crear(libro)) {
							...
							if (Integer.parseInt(numLibros) == 1) {
								...
								alert = new Alert(Alert.SUCCESS,
										"Libro <b>" + libro.getTitulo() + "</b> creado correctamente.");

							} else {
								alert = new Alert(Alert.SUCCESS, "Libro <b>" + libro.getTitulo()
										+ "</b> creado correctamente " + numLibros + " veces.");
							}
						}
					...
					if (servicioLibro.modificar(libro)) {
						alert = new Alert(Alert.SUCCESS,
								"Libro <b>" + libro.getTitulo() + "</b> modificado correctamente.");
					}
				}
