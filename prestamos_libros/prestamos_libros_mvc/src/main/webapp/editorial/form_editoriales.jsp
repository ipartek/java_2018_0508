	...
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 fondo_form">
			<div>
				...
				<h1 class="titulo_form">${(editorial.id == -1)?'Crear':'Editar'} Editorial</h1>
				<form action="editoriales?op=2" method="post">
					<div class="form-group">
						<label>Id:</label>
						<div class="nk-int-st">
							<input name="id" type="text" value="${editorial.id}" required readonly class="form-control input-sm">
						</div>
					</div>
					<div class="form-group">
						<label>Nombre:</label>
						<div class="nk-int-st">
							<input name="nombre" type="text" value="${editorial.nombre}" required autofocus class="form-control input-sm">
						</div>
					</div>
					<button  class="btn btn-success notika-btn-success waves-effect" type="submit">${(editorial.id == -1)?'Crear':'Editar'}</button>
					<c:if test="${editorial.id != -1}">
						<a class="btn btn-danger notika-btn-danger waves-effect" href="editoriales?op=<%=CrudControllable.OP_ELIMINAR %>&id=${editorial.id}" onclick="confirmar(event)">Eliminar</a>
					</c:if>
				</form>
				<script>
