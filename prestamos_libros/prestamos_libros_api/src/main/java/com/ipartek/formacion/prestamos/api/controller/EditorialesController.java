...
@Api(tags = { "Editoriales" }, produces = "application/json", description = "Gestión Editoriales")
@CrossOrigin(origins = "*")
@RestController
...
public class EditorialesController {
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Listado editoriales mostrado") })
	...
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Detalle editorial mostrado", responseContainer = "Editorial"),
	...
	public ResponseEntity<Editorial> detalle(@PathVariable long id) {
		...
		try {
			...
			if (editorial != null && editorial.getId() > 0) {
				...
				LOG.debug("No se ha encontrado la editorial a devolver");
			}
		} catch (Exception e) {
			LOG.error(e);
		}
	...
	@ApiOperation(value = "Crear editorial", notes = "Crea una nueva editorial", produces = "application/json", response = Editorial.class)
	@ApiResponses(value = {
			@ApiResponse(code = 201, message = "Editorial creada", responseContainer = "Editorial"),
			@ApiResponse(code = 409, message = "<ol><li>1) Nombre de editorial ya existe.</li><li> 2) No cumple las validaciones.</li></ol>") })
		...
		try {
			...
			if (violations.isEmpty()) {
				...
				if (serviceEditorial.crear(editorial)) {
					...
					LOG.debug("Conflicto a la hora de crear editorial");
				}
			} else {
				...
				for (ConstraintViolation<Editorial> violation : violations) {
					...
					LOG.debug("Error de validacion al crear editorial: " + violation.getPropertyPath() + ": "
	...
	@ApiOperation(value = "Modificar editorial", notes = "Modifica los valores de una editorial", produces = "application/json", response = Editorial.class)
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Editorial modificada"),
			@ApiResponse(code = 409, message = "<ol><li>1) Nombre de editorial ya existe.</li><li> 2) No cumple las validaciones.</li></ol>") })
		...
		try {
			...
			if (violations.isEmpty()) {
				...
				if (serviceEditorial.modificar(editorial)) {
					...
					response = new ResponseEntity<>(HttpStatus.CONFLICT);
					LOG.debug("No se ha podido modificar la editorial por conflicto");
				}
			} else {
				/* Hay fallos, la Validacion no es correcta */
				ResponseMensaje msj = new ResponseMensaje();
				...
				for (ConstraintViolation<Editorial> violation : violations) {
					...
					LOG.debug("Error de validacion al modificar editorial: " + violation.getPropertyPath() + ": "
							+ violation.getMessage());
				}
				response = new ResponseEntity<>(msj, HttpStatus.CONFLICT);
			}
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Editorial eliminada"),
			@ApiResponse(code = 404, message = "Editorial no existe"),
			@ApiResponse(code = 409, message = "Editorial con libros asociados") })
	...
	public ResponseEntity<Object> eliminar(@PathVariable long id) {
		...
		try {
			if (serviceEditorial.eliminar(id)) {
				...
				LOG.debug("No se ha encontrado la editorial a eliminar");
			}
		} catch (MySQLIntegrityConstraintViolationException e) {
			LOG.debug(e);
