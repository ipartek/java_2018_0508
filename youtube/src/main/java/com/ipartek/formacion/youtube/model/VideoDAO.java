...
public class VideoDAO implements CrudAble<Video> {
	...
	private final String SQL_GET_ALL = "SELECT v.idVideo, v.cod, v.nombre as 'video_nombre' , v.id_usuario"
			+ ", u.idUsuario, u.nombre as 'usuario_nombre', u.password" 
			+ " FROM video as v, usuario as u"
			+ " WHERE v.id_Usuario = u.idUsuario" + " ORDER BY idVideo DESC LIMIT 1000;";

	private final String SQL_GET_BY_ID = "SELECT v.idVideo, v.cod, v.nombre as 'video_nombre' , v.id_usuario"
			+ " , u.idUsuario, u.nombre as 'usuario_nombre', u.password" + " FROM video as v, usuario as u"
			+ " WHERE v.idVideo = ? AND v.id_Usuario = u.idUsuario" + " ORDER BY idVideo DESC LIMIT 1000;";

	private final String SQL_INSERT = "INSERT INTO video (cod, nombre, id_usuario) VALUES (?, ?, ?);";
	private final String SQL_UPDATE = "UPDATE video SET cod = ?, nombre = ?, id_usuario = ? WHERE idVideo = ?;";
