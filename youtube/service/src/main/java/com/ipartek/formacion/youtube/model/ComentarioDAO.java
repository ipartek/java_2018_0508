...
public class ComentarioDAO implements CrudAble<Comentario> {
	...
	private final String SQL_GET_ALL_BY_VIDEO_ID = "SELECT 	c.id as 'id_comentario', c.id_video, u.id as 'id_usuario',    fecha,    texto,    aprobado,    u.nombre as 'nombre_usuario',v.nombre as 'nombre_video' "
			+ " FROM comentario as c , usuario as u,video as v " + " WHERE c.id_usuario = u.id AND "
			+ " c.id_video = ? AND aprobado = 1  AND c.id_video=v.id" + " ORDER BY c.id DESC LIMIT 500;";
