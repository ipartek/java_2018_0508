...
/*Los ladrillos y sus colores y el mx_score*/
var bricks = [];
var colors = [];
...
function registrarEventos() {
	...
	btn_play.addEventListener("click", jugar, false);
}
...
function drawBall() {
	...
	ctx.fillStyle = "rgba(" + colorBall + ", 0.89)";
	ctx.fill();
	ctx.closePath();
}
...
function drawPaddle() {
	...
	ctx.fillStyle = "rgba(" + colorPaddle + ", 1)";
	ctx.fill();
	ctx.closePath();
}
	...
	for (c = 0; c < brickColumnCount; c++) {
		for (r = 0; r < brickRowCount; r++) {
			if (bricks[c][r].status == 1) {
				...
				ctx.fillStyle = "rgba(" + colors[c][r] + ", 0.8)";
				ctx.fill();
				ctx.closePath();
			}
		}
